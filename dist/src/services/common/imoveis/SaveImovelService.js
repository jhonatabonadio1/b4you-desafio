...
exports.SaveImovelService = void 0;
const prismaClient_1 = require("../../../database/prismaClient");
const mongodb_1 = require("mongodb");
class SaveImovelService {
    async execute(data) {
        const { propertyType, quartos, banheiros, garagem, descricao, state, cidade, valor, valorCondo, metragens, images, details, condominium, valorComissao, ownerName, ownerPhone, ownerEmail, ownerWhatsapp, userId, } = data;
        ...
        const metragensWithId = metragens.map((value) => ({
            id: new mongodb_1.ObjectId().toHexString(), // Gera um ID único para cada metragem
            value,
        }));
        // Salva o imóvel no banco de dados
        const property = await prismaClient_1.prismaClient.imovels.create({
            data: {
                v: 0,
                propertyType,
                quartos,
    ...
    async gerarNumeroUnicoDeQuatroDigitos() {
        ...
        while (numeroExiste) {
            ...
            const propriedadeExistente = await prismaClient_1.prismaClient.imovels.findFirst({
...
exports.SaveImovelService = SaveImovelService;
