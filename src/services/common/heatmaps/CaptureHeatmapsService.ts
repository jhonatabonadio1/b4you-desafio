...
class CaptureHeatmapsService {
  async execute({ docId, sessionId, lote }: ICaptureHeatmapsRequest) {
    ...
    if (lastDocSession && lastDocSession.collectedHeatmap) {
      ...
      return {
        message:
          'Este envio foi ignorado devido à alternância de registros para este documento.',
      }
    }
    ...
    if (lastSessionHeatmap) {
      ...
      if (now - lastSessionTime < ONE_MINUTE) {
        throw new Error(
          'Aguarde antes de registrar um novo lote de heatmaps para esta sessão.',
        )
      }
    }
