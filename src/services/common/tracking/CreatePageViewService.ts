...
interface ICreateSessionRequest {
  ...
  fingerprint: string
  network?: string
  pageViews: { pageNumber: number; interactionTime: number }[]
...
class CreatePageViewService {
  async execute({
    ...
    if (!pageViews.length) throw new Error('Nenhuma página informada')
    ...
    if (!buscaSessaoValida) throw new Error('Sessão inválida')

    for (const { pageNumber, interactionTime } of pageViews) {
      if (!pageNumber || interactionTime <= 0) continue // Ignora entradas inválidas
      ...
      if (buscaVisualizacao) {
        ...
        await prismaClient.pageView.update({
          ...
          data: {
            interactionTime:
              buscaVisualizacao.interactionTime + interactionTime,
          },
        })
      } else {
